...
fn add_category(ctx: &mut Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    if args.len() != 1 {
        ...
        ctx.http.get_message(chan.0, msg_to_edit.0)
            .unwrap()
            .edit(&ctx, |m|
                m.content(format_post(guild_games.categories(&guild_id).unwrap())),
            );
    }
...
fn add_game(ctx: &mut Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    if args.len() != 3 {
        let answer = MessageBuilder::new()
            ...
            .push("Use \"")
            .mention(&bot_id)
            .push(" add_game category_name, game_name, emoji\" to add a category")
            .build();
    ...
    fn reaction_add(&self, ctx: Context, added_reaction: Reaction) {
        ...
        if added_reaction.user_id != bot_id {
            ...
            if reacted_msg.author.id == bot_id {
                added_reaction.emoji == ReactionType::Unicode(String::from(TRASHCAN));
